...
# Load models outside the main app logic block
# This prevents the unpacking error when run directly in Colab
loaded_model, encoder, embedder, detector, midas, midas_transform = load_models()


# Check if we are running in a Streamlit environment before proceeding with the UI and processing logic
if streamlit.runtime.exists():
    ...
    if loaded_model is not None and encoder is not None and embedder is not None and detector is not None: # Midas can be None if loading failed
        ...
        if midas is not None and midas_transform is not None:
            @st.cache_data
            def estimate_depth(img, midas_model, transform):
        ...
        @st.cache_data
        def get_embedding(face_img, embedder_model):
            face_img = face_img.astype('float32') # 3D(160x160x3)
            face_img = np.expand_dims(face_img, axis=0)
            yhat= embedder_model.embeddings(face_img)
